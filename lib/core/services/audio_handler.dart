...
class AudiobooklyAudioHandler extends BaseAudioHandler {
  ...
  Duration get currentPositionDuration =>
      Duration(milliseconds: currentPosition ?? 0);
  Duration get currentBufferedPositionDuration =>
      Duration(milliseconds: currentBufferedPosition ?? 0);
  ...
  Future<void> play() async {
    ...
    updateProgress(_player.position.inMilliseconds, PlexPlaybackState.PLAYING);
  }
  ...
  Future<void> pause() async {
    ...
    await super.pause();
    updateProgress(
        _player.position.inMilliseconds - 2000, PlexPlaybackState.PAUSED);
  ...
  Future<void> stop() async {
    updateProgress(_player.position.inMilliseconds, PlexPlaybackState.STOPPED);
    await _player?.stop();
    await super.stop();
  }
  ...
  Future<MediaItem> getMediaItem(String mediaId) {
    return _communicator.getAlbumFromId(mediaId);
  }

  @override
  Future<void> prepare() async {
    playFromMediaId(
        (await _communicator.getRecentPlayed()).take(1).toList()[0].id);
  }
  ...
  Future<void> prepareFromMediaId(String mediaId,
    ...
    if (_player?.playing ?? false) {
      updateProgress(
          _player.position.inMilliseconds, PlexPlaybackState.STOPPED);
      _player.stop();
    }
    await _communicator.getServerAndLibrary();
  ...
  Future<void> playFromSearch(String query,
      [Map<String, dynamic> extras]) async {
    List<MediaItem> items = await _communicator.search(query);
  ...
  Future<List<MediaItem>> search(String query,
      [Map<String, dynamic> extras]) async {
    return await _communicator.search(query);
  }
